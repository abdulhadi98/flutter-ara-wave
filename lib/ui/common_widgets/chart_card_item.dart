...
class Xclass {
  ...
  Xclass({DataStore? dataStore, List<SalesData>? chartData})
      : _chartData = chartData,
        _dataStore = dataStore;
  UserModel? get currentUser => _dataStore!.userModel;
  void getToken() async {
    String? api = await _dataStore!.getApiToken();
    //apiToken = api;
  }
  ...
  Future<void> getPersonalChartValue() async {
    ...
    var response;
    var url = Uri.parse(
      'https://wave.aratech.co/api/get-personal-assets-chart-data',
    );
...
class ChartCardItem extends BaseStateFullWidget {
  ...
  final chartTitleKey;
  final List<int>? personalGrothValue;
  final List<PublicAssetGraphModel>? historicalDataList;
  ChartCardItem({
    required this.chartType,
    this.chartTitleKey,
    this.personalGrothValue,
...
class _ChartCardItemState extends BaseStateFullWidgetState<ChartCardItem> {
  final BehaviorSubject<ChartFilters> filterController =
      BehaviorSubject<ChartFilters>.seeded(ChartFilters.xALL);
                ...
                children: [
                  ...
                  Padding(
                    ...
                    child: Row(
                      ...
                      children: [
                        buildChartFilterItem(
                          ChartFilters.x24H,
                          filterSnapshot.data == ChartFilters.x24H,
                        ),
                        buildChartFilterItem(
                          ChartFilters.x7D,
                          filterSnapshot.data == ChartFilters.x7D,
                        ),
                        buildChartFilterItem(
                          ChartFilters.x1M,
                          filterSnapshot.data == ChartFilters.x1M,
                        ),
                        buildChartFilterItem(
                          ChartFilters.x3M,
                          filterSnapshot.data == ChartFilters.x3M,
                        ),
                        buildChartFilterItem(
                          ChartFilters.x1Y,
                          filterSnapshot.data == ChartFilters.x1Y,
                        ),
                        buildChartFilterItem(
                          ChartFilters.xALL,
                          filterSnapshot.data == ChartFilters.xALL,
                        ),
                      ],
                    ),
                  ),
    ...
    return GestureDetector(
      ...
      child: Container(
        padding: EdgeInsets.symmetric(
            horizontal: width * .02, vertical: height * .006),
        decoration: BoxDecoration(
        ...
        series: <CartesianSeries<SalesData, int>>[
          ...
          ColumnSeries<SalesData, int>(
            ...
            dataLabelSettings: DataLabelSettings(
              isVisible: widget.chartTitleKey != 'projected_flow_cash'
                  ? true
                  : false, //TODO
              textStyle: TextStyle(
                  color: Colors.white,
                  fontSize: AppFonts.getXXSmallFontSize(context)),
              labelAlignment: widget.chartTitleKey != null
