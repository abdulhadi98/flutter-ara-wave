...
class AddPersonalAssetDocumentStepWidgetController {
  final AddPersonalAssetDocumentStepWidgetBloc _addPersonalAssetDocumentStepWidgetBloc;
  AddPersonalAssetDocumentStepWidgetController({required addPersonalAssetDocumentStepWidgetBloc})
      : _addPersonalAssetDocumentStepWidgetBloc = addPersonalAssetDocumentStepWidgetBloc;

  final BehaviorSubject<bool> _validationController = BehaviorSubject.seeded(true);
  ...
  onFinishedClicked({
    ...
    if(getPickedPhotoAssets()?.isNotEmpty??false) {
      ...
      _addPersonalAssetDocumentStepWidgetBloc.uploadImageList(
        images: images,
        onData: (imageUrlList) => _onHoldingAddingSucceed(imageUrlList, onFinishedClicked),
  ...
  _onHoldingAddingSucceed(List<String>? imageUrlList, onFinishedClicked) {
  ...
  List<UploadImageModel> _createUploadImageList(assetType){
    return getPickedPhotoAssets()!.map<UploadImageModel>((e) => UploadImageModel(
      ...
      assetType: Utils.enumToString(HoldingsType.PERSONAL).toLowerCase(),
      apiToken: _addPersonalAssetDocumentStepWidgetBloc.currentUserApiToken??'',
    )).toList();
  }
    ...
    Utils.getImagesFromGallery(
      onData: (List<XFile>? xFiles) {
        if(xFiles!=null && xFiles.isNotEmpty){
          if(getPickedPhotoAssets()!=null) setPickedPhotoAssets(getPickedPhotoAssets()?..addAll(xFiles));
          else setPickedPhotoAssets(xFiles);
        }
      },
      onError: () {
