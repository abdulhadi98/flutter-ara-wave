...
class TopPerformingGainersLoosersModel {
  ...
  factory TopPerformingGainersLoosersModel.fromJson(
          Map<String, dynamic> json) =>
      TopPerformingGainersLoosersModel(
        topPerforming: List<GainersLooser>.from(
            json["top_performing"].map((x) => GainersLooser.fromJson(x))),
        gainersLoosers: List<GainersLooser>.from(
            json["gainers_loosers"].map((x) => GainersLooser.fromJson(x))),
      );

  Map<String, dynamic> toJson() => {
        "top_performing":
            List<dynamic>.from(topPerforming.map((x) => x.toJson())),
        "gainers_loosers":
            List<dynamic>.from(gainersLoosers.map((x) => x.toJson())),
      };
}
...
class GainersLooser {
  ...
  String get growthTitle =>
      (growth.toInt() < 0 ? '' : '+') + "${Utils.getFormattedNum(growth)}%";
  ...
  factory GainersLooser.fromJson(Map<String, dynamic> json) => GainersLooser(
        ...
        absGrowth:
            json["abs_growth"] == null ? null : json["abs_growth"].toDouble(),
      );
